#'Calculates per share Profit and Loss (PnL) at expiration for Long Put Condor Option Strategy and draws its Bar Plot displaying PnL in the Plots tab.
#'@description
...
#'@param X1L Lower Strike Price or eXercise price for one ITM bought put.
#'@param X2Ml Middle-low Strike Price or eXercise price for middle strike sold put.
#'@param X3Mu Middle-upper Strike Price or eXercise price for middle strike sold put.
#'@param X4H Higher Strike Price or eXercise price for one OTM bought put.
#'@param P1L Put Premium or Put Price paid for the one OTM bought Put.
#'@param P2Ml Put Premium or Put Price received from the middle-low sold Put.
#'@param P3Mu Put Premium or Put Price received from the middle-upper sold Put.
#'@param P4H Put Premium or Put Price paid for the one ITM bought Put.
...
#'@export
longPutCondor <- function (ST,X1L,X2Ml,X3Mu,X4H,P1L,P2Ml,P3Mu,P4H,hl=0,hu=2,spot=spot,pl=pl,myData=myData,myTibble=myTibble,PnL=PnL){
  V0= -P1L-P4H + P2Ml +P3Mu
  myData <- data.frame (spot = c((ST*hl):(ST*hu)))
  myData$pl <- (-pmax((X2Ml-myData$spot),0))+(-pmax((X3Mu-myData$spot),0))+ (pmax((X1L-myData$spot),0))+ (pmax((X4H-myData$spot),0)) + V0
  ...
  ggplot(myTbColored,aes(x=spot,y=pl,fill=PnL,label=pl)) +
    ...
    geom_text(nudge_y = 0.7,size= 3, color="black")+
    ...
    labs(title  =  'Long Put Condor Strategy', x  =  'Spot Price ($) at Expiration', y  =  'PnL ($) at Expiration', caption  =  'condorOptions / MaheshP Kumar')
}
