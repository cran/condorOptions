...
#'@examples
#'shortCallCondor(415,400,420,440,460,50,35,22,16,hl=0.95,hu=1.125)
#'@export
shortCallCondor <- function (ST,X1L,X2Ml,X3Mu,X4H,C1L,C2Ml,C3Mu,C4H,hl=0,hu=2,spot=spot,pl=pl,myData=myData,myTibble=myTibble,PnL=PnL){
  ...
  ggplot(myTbColored,aes(x=spot,y=pl,fill=PnL,label=pl)) +
    ...
    labs(title  =  'Short Call Condor Strategy', x  =  'Spot Price ($) at Expiration', y  =  'PnL ($) at Expiration', subtitle  =  'High Volatility / Neutral Outlook', caption  =  'condorOptions / MaheshP Kumar')
}
